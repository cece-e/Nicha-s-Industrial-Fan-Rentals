...
window.hidePreview = function () {
    ...
    confirmBtn.addEventListener("click", async () => {
        try {
            ...
            // Using the new safe variable name here
            if (!supabaseDb) {
...
async function generatePDF() {
    ...
    await new Promise((resolve) => {
        logo.onload = resolve;
        logo.onerror = () => {
            console.warn("logo.jpg could not be loaded. Continuing PDF assembly without it.");
            resolve();
        };
    });
    ...
    try {
        if (logo.complete && logo.naturalWidth > 0) {
            doc.addImage(logo, "JPEG", 55, 5, 100, 50);
        }
    } catch (e) {
        console.error("Failed to append logo image to canvas context:", e);
    }

    //page 1

    doc.setFontSize(18);
    doc.text("NICHA'S INDUSTRIAL FAN RENTALS", 105, 65, { align: "center" });

    doc.setFontSize(10);
    doc.text("3508 VIGAN STREET, BRGY. 579, STA. MESA, MANILA, 1008", 105, 72, { align: "center" });
    doc.text("09565322670", 105, 78, { align: "center" });
    ...
    y += 15;
        const proposalText = [
        "Nicha's Industrial Fan Rentals is honored to be given the opportunity to meet your requirements. We sincerely hope that you will find our proposal well-suited to your needs.",
        "With our high-quality equipment, technical expertise, and dedicated support team, we are fully committed to ensuring the success and smooth execution of your event or project.",
        "Should you have any questions or require further clarification, please feel free to contact our assigned Account Manager, who will be more than happy to assist you.",
        "Our Financial Proposal, along with the Terms and Conditions, is attached for your review and consideration.",
        "Thank you very much for considering Nicha's Industrial Fan Rentals for this undertaking. We look forward to the opportunity to serve you.",
        "",
        "Very truly yours,",
    ...
    proposalText.forEach(paragraph => {
        if (paragraph === "") {
            y += 4;
            return;
        }
        const wrappedLines = doc.splitTextToSize(paragraph, 180);
    ...
    doc.autoTable({
        ...
        body: [
            ...
            ["2", "Set Up and Pull Out", currentData.quantity, "FREE", "FREE"],
            ["3", "Delivery and Return Fee", "1", `Php ${currentData.delivery.toLocaleString()}`, `Php ${currentData.delivery.toLocaleString()}`],
            ["4", "Extension Cord", currentData.cordQty, "Php 150.00", `Php ${currentData.cordTotal.toLocaleString()}`],
            ["5", "Standby Personnel", currentData.days, "Php 1,000.00", `Php ${currentData.standbyTotal.toLocaleString()}`]
        ]
    });

    y = doc.lastAutoTable.finalY + 10;
    doc.setFontSize(11);
    doc.text(`Discount: Php ${currentData.discount.toLocaleString()}`, 15, y);
    y += 8;
    doc.text(`VAT: Php ${currentData.vat.toFixed(2)}`, 15, y);
    y += 10;
    doc.setFontSize(14);
    doc.text(`GRAND TOTAL: Php ${currentData.total.toLocaleString()}`, 15, y);

    y += 15;
    doc.setFontSize(11);
    doc.text("ANY LOSS OR DAMAGE TO THE DELIVERED ITEM/S WILL BE CHARGED ACCORDINGLY:", 15, y);
    y += 8;
    doc.text("Industrial Fan = Php 8,000 each", 15, y);
    y += 6;
    doc.text("Extension Cord = Php 400 each", 15, y);
    ...
    const sections = [
        { title: "Booking Confirmation and Payment Terms", 
            body: "50% upon confirmation (or Php 1,000.00 only if total is under 10,000) plus 1 government I.D and 50% (or the rest of the balance) upon delivery (For Strict Compliance). We only provide an Acknowledgement Receipt for our clients. If the client wants an Official Receipt, there is an additional 8% for VAT." },
        { title: "Order Cancellation", 
            body: "50% down payment NON-REFUNDABLE." },
        { title: "Power Requirements",
            body: "Electrical supply and necessary cables will be provided by the client." },
        { title: "Delivery", 
            body: "The client should inform the supplier of the event's exact location like room/s, floor/s, indoor or outdoor. If the venue is located on the upper floor." },
        { title: "Meals (Only if Client Requested a Standby Crew)", 
            body: "Meals for the supplier's personnel will be provided by the client or will add Php150 per pax per meal. The supplier will inform the client of the number of personnel needed to set up the event." },
        { title: "Return Policy", 
            body: "All items must be returned to Nicha's Industrial Fan Rentals after the event duration. Failure to do so will result in additional payment of rented items per day and forfeiture of the security deposit." }
    ];

    sections.forEach(section => {
        doc.setFontSize(12);
        doc.text(section.title + ":", 15, termsY);
        termsY += 7;

        doc.setFontSize(11);
        const wrapped = doc.splitTextToSize(section.body, 180);
        doc.text(wrapped, 15, termsY);
    ...
    doc.setFontSize(11);
    const conformeText = "This Proposal constitutes a valid and binding contractual agreement once signed by you. Your signature below indicates acceptance of the quotation, terms, and conditions stated in this document.";
    const conformeLines = doc.splitTextToSize(conformeText, 180);
    doc.text(conformeLines, 15, 40);

    doc.text("CONFORME:", 15, 120);
    doc.line(50, 118, 140, 118);
    doc.text("Authorized Representative", 15, 135);
    doc.text("(Signature over Printed Name)", 15, 142);
